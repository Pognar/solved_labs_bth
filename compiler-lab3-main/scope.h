...
public:
    ...
    void print_scope(){
        cout << "-----------------Scope-------------" << endl;
        cout << "Records: " << endl;
        unordered_map<string, Record*>::iterator i;
    ...
    Record* print_kids_per_method(std::string Key){
        unordered_map<string, Record*>::iterator i;
        for (i = records.begin(); i != records.end(); i++) {
            if(i->first == Key){
                if (i->second->getRecord() == "Class" || i->second->getRecord() == "Method") {
                    return i->second; 
                }
            }
        }
        vector<Scope*>::iterator it;
        for(it = children_scopes.begin(); it != children_scopes.end(); it++) {
            Record* r = (*it)->print_kids_per_method(Key);
            if (r != nullptr) {
                return r;
            }
        }
